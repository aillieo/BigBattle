...
{
    public class SDirectionSystem : ReactiveSystem<ServerEntity>
    {
        readonly ServerContext _context;

        public SDirectionSystem(Contexts contexts) : base(contexts.server)
        {
        ...
        {
            return entity.hasTargetPos && entity.hasDirection;
        }

        protected override ICollector<ServerEntity> GetTrigger(IContext<ServerEntity> context)
        {
            return context.CreateCollector(ServerMatcher.AnyOf(ServerMatcher.TargetPos, ServerMatcher.Direction));
        }
    }
}
